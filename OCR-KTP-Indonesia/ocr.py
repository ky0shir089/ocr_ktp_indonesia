    ...
    for tmp_data in last_result_list:
        ...
        if 'RT/RW' in tmp_data:
            for tmp_index in range(len(tmp_data)):
                ...
                if "i" in tmp_data[tmp_index]:
                    tmp_data[tmp_index] = tmp_data[tmp_index].replace("“", "")
                rt_rw = ' '.join(tmp_data[2:])
        ...
        if 'Berlaku' in tmp_data or 'Hingga' in tmp_data:
            berlaku_hingga = ' '.join(tmp_data[3:])

    # for tmp_data in last_result_list:
    #     print(' '.join(tmp_data))

    return (id_number, nama, tempat_lahir, tgl_lahir, jenis_kelamin, gol_darah, 
